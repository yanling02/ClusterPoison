...
from utils import EarlyStopping, get_user_seqs_long, get_user_seqs_and_sample, get_item2attribute_json, check_path, set_seed
...
def main():
    ...
    args.poison_data_file = args.data_dir + args.data_name + '.txt'
    args.sample_file = args.data_dir + args.dataset + '_sample.txt'
    item2attribute_file = args.data_dir + args.dataset + '_item2attributes.json'

    user_seq_poison, _, _ = get_user_seqs_long(args.poison_data_file)

    user_seq, max_item, sample_seq = \
        get_user_seqs_and_sample(args.data_file, args.sample_file)

    sample_seq = [sample_s[:-1] + [user_s[-1]] for user_s, sample_s in zip(user_seq, sample_seq)]
    ...
    train_dataset = SASRecDataset(args, user_seq_poison, data_type='train')
