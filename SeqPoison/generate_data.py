...
def generate_data():
    ...
    gen.load_state_dict(torch.load(f"./output/output_Beauty_Kmeans_max_8/attack-generator-epochs-{i}.pt"))
    datas = []

    while True:
        data = gen.sample(10).cpu().data.numpy().tolist()
        data = unpadding(data,start_id=start_id, target_item=8887) #target item
        datas.extend(data)
        start_id = start_id + len(data)
        if len(datas) > int(data_num * percentage):
            break

    with open(f"./output/generate_epoch{i}_data_{str(percentage)}.txt", 'w') as fout:
        for sample in datas:
